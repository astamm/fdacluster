...
plot.kma <- function(x, bp_sim = FALSE, wr_fun = FALSE, ...) {
  ...
  for (l in 1:n.dim) {
    ...
    matplot(t(x.final), t(y0[, , l]),
      type = "l", lwd = lwd.functions, col = colours.bygroup.dark,
      xlab = "x", ylab = tex
    )
    ...
    legend("topleft",
      legend = text, col = colours.templates.last,
      lty = lty, cex = 0.6
    )
  }
  ...
  if (wr_fun == TRUE) {
    ...
    plot(t(x[1, ]), t(x.final[1, ]),
      xlim = c(
        ...
        min(x, na.rm = TRUE),
        max(x, na.rm = TRUE)
      ), type = "l", lwd = 1, col = colours.warping[1],
      xlab = "x", ylab = "y", asp = 1
    )
    for (i in 2:n.obs) {
      lines(t(x[i, ]), t(x.final[i, ]),
        type = "l", lwd = 1,
  ...
  if (bp_sim == TRUE) {
    if (cen_or == TRUE) {
      ...
      if (similarity.method == "pearson") {
        boxplot(-sims.orig, -sims.final,
          notch = FALSE, boxwex = 0.3,
          col = c("grey", "orange"), ylim = c(min(
            -sims.orig,
            -sims.final
          ), 1)
        )
      ...
      if (similarity.method == "l2") {
        boxplot(sims.orig, sims.final,
          notch = FALSE, boxwex = 0.3,
          col = c("grey", "orange"), ylim = c(
            min(sims.orig, sims.final),
      ...
      lty <- rep(1, length(text))
      legend(
        x = min(x), y = max(x), legend = text, col = colours.templates.last,
        lty = lty, cex = 0.6
      )
      etichette <- rep(0, 2)
